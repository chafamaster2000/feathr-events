...
const STAGES: Stage[] = [
  ...
  },
  {
    ...
    owns: 'In-process: a variable in the API process’s memory, not a service. It owns delivery semantics: visibility timeout, delivery counting, backoff, dead-letter routing. A retry here is the absence of a delete, not an action.',
  },
  {
    id: 'worker',
    label: 'Worker ×N',
    x: 468, y: 96, w: 116, h: 52,
    owns: 'Asyncio tasks, not processes. They share this one process and its single event loop, taking turns while each other waits on I/O, so the count is concurrency and not a number of machines. It needs no lock because the work is commutative: every event carries its own timestamp and the upsert is idempotent by event_id. Writes MongoDB, then Elasticsearch, then acknowledges. It contains no retry logic: if a write fails it never reaches the delete, and the message returns on its own.',
  },
  {
    id: 'mongo',
    label: 'MongoDB',
  ...
  },
  {
    ...
    step: 2,
    owns: 'A derived index, never authoritative. Losing it is lag rather than loss, because it rebuilds from MongoDB. It refreshes once per second, which is the gap you see before a traced event becomes searchable.',
  },
  {
    id: 'redis',
    label: 'Redis',
